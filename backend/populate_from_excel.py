#!/usr/bin/env python3
"""
Script to populate database from Excel file
"""
import sys
import openpyxl
from app import app
...
def read_excel_data(filepath):
    ...
    for row in ws.iter_rows(min_row=2, values_only=True):
        if row and len(row) >= 4:
            data.append({
                'customer': str(row[0]).strip() if row[0] else None,
                'executor': str(row[1]).strip() if row[1] else None,
                'driver': str(row[2]).strip() if row[2] else None,
                'phone': str(row[3]).strip() if row[3] else None
            })
    return data

def populate_database(excel_path):
    """Populate database with data from Excel"""
    with app.app_context():
        ...
        for row in data:
            if row['customer'] and row['customer'] != 'None':
        ...
        for row in data:
            ...
            if (driver_name and driver_name != 'None' and
                executor_name and executor_name != 'None' and
                ...
                if not existing:
                    ...
                    driver_count += 1
                    print(f"  Added: {driver_name} ({executor_name}) - {phone}")
...
if __name__ == '__main__':
    excel_path = '../database temp/DB1.xlsx'
